...
const renderCampaigns = () => {
  ...
  const list = allCampaigns.filter((campaign) => {
  ...
  const renderStatusOptions = (current) =>
    Object.keys(statusLabels)
      .map((key) => `<option value="${key}" ${key === current ? 'selected' : ''}>${statusLabels[key]}</option>`)
      .join('');
          ...
          ${list
            .map((campaign) => {
              ...
              const stageOptionsHtml = stageOptions.length
                ? stageOptions
                    .map((opt) => `<option value="${opt.id}" ${opt.id === stageSafe ? 'selected' : ''}>${opt.label}</option>`)
                    .join('')
                : '<option value="">-</option>';

              return `
                <tr data-campaign-id="${campaign.id}">
                  <td data-label="Marca"><span class="chip chip-pill brand-status-${statusSafe}">${escapeHtml(campaign.brand || 'Marca')}</span></td>
                  <td data-label="Prazo">${escapeHtml(formatDateShortBR(campaign.dueDate))}</td>
                  <td data-label="Preço">${escapeHtml(getValueLabel(campaign))}</td>
...
const renderMetrics = () => {
    ...
    hero.innerHTML = `
      <div class="metric-hero">
        <div class="metric-hero-title">
          ...
          <p class="muted">Você tem <strong>${metrics.totalCampaigns}</strong> campanhas rolando agora.</p>
        </div>
        <div class="metric-hero-badges">
          <span class="chip chip-pendente">Prospecção: ${metrics.prospeccaoCount}</span>
          <span class="chip chip-negociando">Produção: ${metrics.producaoCount}</span>
          <span class="chip chip-pay-partial">Finalização: ${metrics.finalizacaoCount}</span>
          <span class="chip chip-realizado">Concluída: ${metrics.doneCount}</span>
        </div>
      </div>
    `;
  }
  ...
  container.innerHTML = `
    ...
    <div class="card metric-card">
      ...
      <p class="muted">Tudo que ainda não virou pago.</p>
    </div>
    <div class="card metric-card">
    ...
    <div class="card metric-card">
      ...
      <div class="muted">Média por campanha</div>
    ...
    <div class="card metric-card">
      ...
      <p class="muted">Da criação até pagar.</p>
    </div>
  `;

  const funnel = [
    { label: 'Prospecção', value: metrics.prospeccaoCount },
    { label: 'Produção', value: metrics.producaoCount },
    { label: 'Finalização', value: metrics.finalizacaoCount },
    { label: 'Concluída', value: metrics.doneCount }
  ];
  ...
  if (insightsContainer) {
    ...
    if (metrics.prospeccaoCount > 0) {
      insights.push({
        icon: 'send',
        title: 'Destrava a prospecção',
        text: `Você tem ${metrics.prospeccaoCount} campanha(s) em prospecção. Faz 1 follow-up hoje e já melhora o jogo.`
      });
    }
    if (metrics.negociacaoCount > 0) {
      insights.push({
        icon: 'chat',
        title: 'Negociação quente',
        text: `Tem ${metrics.negociacaoCount} em negociação. Manda um pacote simples (2 vídeos + 3 cortes) e fecha.`
      });
    }
    if (metrics.doneCount === 0) {
      insights.push({
        icon: 'radar',
        title: 'Primeira paga do mês',
        text: 'Escolhe 1 marca e vai até o "sim". Uma vitória puxa a outra.'
      });
    }
    while (insights.length < 3) {
      insights.push({
        icon: 'trend',
        title: 'Ritmo constante',
        text: 'Todo dia um passo: roteiro, contato, follow-up. Sem pressão, só consistência.'
      });
    }
