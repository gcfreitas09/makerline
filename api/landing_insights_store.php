...
/**
 * Grava o evento no espelho local. O retorno diz se deu certo; quem chama
 * decide o que fazer, porque o registro durável fica no Supabase e nao pode
 * depender do arquivo local.
 */
function landing_insights_write_local_event($event, $file = LANDING_INSIGHTS_STORAGE_FILE)
{
    if (!landing_insights_ensure_storage_file($file)) {
...
{
    $local = landing_insights_write_local_event($event, $file);
    $localOk = !empty($local['ok']);

    // O remoto roda mesmo com o espelho local quebrado: disco cheio ou
    // permissao errada nao podem derrubar o registro durável no Supabase.
    $remoteOk = landing_insights_remote_upsert_event($event);

    if (!$localOk && !$remoteOk) {
        return [
            'ok' => false,
            'error' => $local['error'] ?? 'Nao consegui salvar o evento.',
            'remoteError' => landing_insights_remote_last_error(),
        ];
    }

    return [
        'ok' => true,
        'replaced' => !empty($local['replaced']),
        'count' => $local['count'] ?? null,
        'updatedAt' => $local['updatedAt'] ?? landing_insights_iso_now(),
        'localOk' => $localOk,
        'localError' => $localOk ? null : ($local['error'] ?? null),
        'remoteOk' => $remoteOk,
