...
function supabase_info($config, $reason, $error = null)
{
    ...
    if ($config) {
        $safe['supabase'] = [
            ...
            'table_pre_signups' => $config['table_pre_signups'] ?? 'landing_pre_signups',
...
function load_supabase_config()
{
    ...
    if (!file_exists($file)) {
        ...
        $tablePreSignups = trim((string)(getenv('UGC_SUPABASE_PRE_SIGNUPS_TABLE') ?: 'landing_pre_signups'));
        ...
        if ($url !== '' && $key !== '') {
            $config = [
                ...
                'table_pre_signups' => $tablePreSignups !== '' ? $tablePreSignups : 'landing_pre_signups',
    ...
    $tablePreSignups = trim((string)($data['table_pre_signups'] ?? ($data['table_landing_waitlist'] ?? 'landing_pre_signups')));
